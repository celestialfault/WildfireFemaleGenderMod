...
public class GenderLayer extends FeatureRenderer<AbstractClientPlayerEntity, PlayerEntityModel<AbstractClientPlayerEntity>> {
	...
	private void renderArmorTrim(ArmorMaterial material, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int packedLightIn,
		...
		// FIXME as-is, this doesn't render the enchantment glint; replacing getDirectItemGlintConsumer() with
		//       getArmorGlintConsumer() does result in the glint rendering, but not in sync with the rest of the armor,
		//       which is a solution I'd personally consider to be worse than simply leaving this without a glint,
		//       especially given I don't understand Minecraft's rendering system enough to even try to fix this.  - celeste
		// (though, it's also not like it really matters all that much, with vanilla's enchant glint having been made
		// incredibly weak now)
