...
public class HolidayFeaturesRenderer extends FeatureRenderer<PlayerEntity, BipedEntityModel<PlayerEntity>> {
	...
	public HolidayFeaturesRenderer(FeatureRendererContext<PlayerEntity, BipedEntityModel<PlayerEntity>> context) {
		super(context);
	...
	public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, PlayerEntity entity, float limbAngle, float limbDistance, float tickDelta, float animationProgress, float headYaw, float headPitch) {
		...
		renderSantaHat(entity, matrices, vertexConsumers, light);
	}

	private void renderSantaHat(PlayerEntity entity, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int light) {
		if(!christmas) return;
		...
		try {
			...
			if(entity.isBaby()) {
				var accessor = (AnimalModelAccessor) getContextModel();
